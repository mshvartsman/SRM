...
for itr in range(args.niter):
    ...
    if args.exptype == 'idvclas':
        ...
        for label, pred_data in enumerate([pred_data_group1, pred_data_group2]):
            ...
            if label == 0:
                print('\nerr1:{:.3f}, err2:{:.3f}, err2-err1:{}'.format(err1,err2,err2-err1), end=' ')
            else:
                print('err1:{:.3f}, err2:{:.3f}, err1-err2:{}'.format(err1,err2,err1-err2), end=' ')
        ...
        for i in range(W_group1.shape[2]):
        ...
        for i in range(align_data_group1_loo.shape[2]):
